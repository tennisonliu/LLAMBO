...
class GpBoOptimizer:
    def __init__(self, hyperparameter_constraints, lower_is_better=False):
        self.lower_is_better = lower_is_better
        self.optimizer = Optimizer(
            hyperparameter_constraints, n_initial_points=0, avoid_duplicates=True
        )
        self.hyperparameter_constraints = hyperparameter_constraints

    def tell(self, new_config, new_fval):
        """Updates optimizer state"""
        for i in range(new_config.shape[0]):
            self.optimizer.tell(
                new_config.iloc[i].to_list(),
                (
                    new_fval["score"].values[i]
                    if self.lower_is_better
                    else -new_fval["score"].values[i]
                ),
            )

    def ask(self):
        """Computes next candidate point"""
        new_values_dict = dict(
            zip(
                [dim.name for dim in self.hyperparameter_constraints],
                self.optimizer.ask(),
            )
        )
        df = pd.DataFrame([new_values_dict])
        return df


class MabController:
    def __init__(self, budget, costs, T, warm_up=4, gamma=0.5):
        self.randoms = np.random.rand(100)
        self.budget = budget
        self.costs = costs
        self.mean_cheap = 0
        self.mean_expensive = 0
        self.counter = 0
        self.warm_up = warm_up
        self.gamma = gamma
        self.T = T
        self.budgets = [budget] * 5
        self.choices = [np.nan] * 5
        self.cheap_means = [np.nan] * 5
        self.expensive_means = [np.nan] * 5
        self.probabilities = [np.nan] * 5
        self.rewards = [np.nan] * 5

    def make_decision(self):
        self.cheap_means.append(self.mean_cheap)
        self.expensive_means.append(self.mean_expensive)

        if self.counter < self.warm_up:
            choice = (self.counter + 1) % 2
            print(
                f"[MabController] - Warm-up. Choose both options in alternation. Choice {choice}"
            )
            self.choices.append(choice)
            self.probabilities.append(choice)
            return choice
        if self.costs > self.budget:
            choice = 0
            print(
                "[MabController] - Trivial case: No budget left -> choose cheap choice"
            )
            self.choices.append(choice)
            self.probabilities.append(choice)
            return choice
        if self.mean_cheap >= self.mean_expensive:
            choice = 0
            print(
                "[MabController] - Trivial case: Cheap is better -> choose cheap choice"
            )
            self.choices.append(choice)
            self.probabilities.append(choice)
            return choice
        prob = self.budget / (self.T * self.costs)
        self.probabilities.append(prob)
        x = self.randoms[self.counter]
        if x < prob:
            choice = 1
        else:
            choice = 0
        print(
            f"[MabController] - Complicated case: Balance costs and reward probabilistically. Prob {prob}, choice {choice}"
        )
        self.choices.append(choice)
        return choice

    def update(self, reward, choice):
        """Update metrics. Exp. Smoothing for mean estimates."""
        if choice == 0:
            self.mean_cheap = (1 - self.gamma) * self.mean_cheap + (self.gamma * reward)
        else:
            self.mean_expensive = (1 - self.gamma) * self.mean_expensive + (
                self.gamma * reward
            )
            self.budget -= self.costs
...
class LLAMBO:
    def __init__(
        ...
        gp_bo_optimizer: Optional[GpBoOptimizer] = None,
        mab_controller: Optional[MabController] = None,
        ...
        if gp_bo_optimizer:
            assert (
                mab_controller
            ), "Mab Controller is necessary when using intermittent GBO"
        if mab_controller:
            assert (
                gp_bo_optimizer
            ), "GP Optimizer is necessary when using intermittent GBO"
        self.gp_bo_optimizer = gp_bo_optimizer
        ...
        rate_limiter = RateLimiter(
            max_tokens=20000000, time_frame=60, max_requests=20000
        )
    ...
    def _initialize(self):
        ...
        if self.gp_bo_optimizer:
            self.gp_bo_optimizer.tell(self.observed_configs, self.observed_fvals)

        print(
            f"[Initialization] COMPLETED: {self.observed_fvals.shape[0]} points evaluated..."
        )
        end_time = time.time()
    ...
    def _update_observations(self, new_config, new_fval):
        ...
        if self.gp_bo_optimizer:
            self.gp_bo_optimizer.tell(new_config, new_fval)
    ...
    def optimize(self, test_metric="generalization_score"):
        ...
        for trial_id in range(self.n_trials):
            ...
            choice = self.mab_controller.make_decision() if self.mab_controller else 1
            ...
            if choice == 1:
                ...
                sel_candidate_point, cost, time_taken = (
                ...
                sel_candidate_point = self.gp_bo_optimizer.ask()
            ...
            sel_candidate_point, sel_candidate_fval = self._evaluate_config(
                sel_candidate_point
            )
            ...
            if self.lower_is_better:
                if self.mab_controller:
                    reward = max(self.best_fval - current_fval_cv, 0)
                    self.mab_controller.update(reward, choice)
                ...
                if self.mab_controller:
                    reward = max(current_fval_cv - self.best_fval, 0)
                    self.mab_controller.update(reward, choice)
                if current_fval_cv > self.best_fval:
                    self.best_fval = current_fval_cv
                    best_found = True
...
__all__ = ["LLAMBO", "GpBoOptimizer", "MabController"]
