...
class LLM_ACQ:
    ...
    def _jitter(self, desired_fval):
        ...
        assert hasattr(
            self, "observed_best"
        ), "observed_best must be set before calling _jitter"
        assert hasattr(
            self, "observed_worst"
        ), "observed_worst must be set before calling _jitter"
        ...
        if observed_configs is not None:
            ...
            for index, row in observed_configs.iterrows():
                ...
                for i in range(len(row)):
                    ...
                    n_dp = 5  # We set the number of decimals to 5 at all times due to a bug in decimal counting when reading information from hp_configurations&/bayesmark.json
                    value = row[i]
...
Hyperparameter configuration: {Q}"""
            ...
            example_prompt = PromptTemplate(
                input_variables=["Q", "A"], template=example_template
            )
            ...
            for i, (hyperparameter, constraint) in enumerate(
...
Hyperparameter configuration:"""
            ...
            for retry in range(MAX_RETRIES):
                try:
                    ...
                    self.rate_limiter.add_request(
                        request_text=user_message, current_time=start_time
                    )
                    ...
                    resp = resp.to_dict()
                    self.rate_limiter.add_request(
                        request_token_count=resp["usage"]["total_tokens"],
    ...
    def _filter_candidate_points(self, observed_points, candidate_points, precision=8):
        ...
        rounded_observed = [
            {key: round(value, precision) for key, value in d.items()}
        ...
        def is_within_range(value, allowed_range):
            ...
            if value_type == "int":
                ...
                value_type == "float"
            ):  # THIS MIGHT NEED TO CHANGE, RIGHT NOW IT CAN"T SIT ON THE BOUNDARY
                [min_val, max_val] = search_range
                if transform == "log" and self.apply_warping:
                    min_val = np.log10(min_val)
                    max_val = np.log10(max_val)
                ...
                return any(
                    math.isclose(value, x, abs_tol=1e-2) for x in allowed_range[2]
                )
            else:
                raise Exception("Unknown hyperparameter value type")
