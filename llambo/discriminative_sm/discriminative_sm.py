...
from llambo.discriminative_sm.discriminative_sm_utils import gen_prompt_tempates
...
class LLM_DIS_SM:
    ...
    async def _async_generate(self, few_shot_template, query_example, query_idx):
        ...
        async with ClientSession(trust_env=True) as session:
            resp = None
            n_preds = (
                int(self.n_gens / self.n_templates)
                if self.bootstrapping
                else int(self.n_gens)
            )
    ...
    async def _predict(self, all_prompt_templates, query_examples):
        ...
        for i in range(0, len(query_examples), 5):
            ...
            chunk_results = await self._generate_concurrently(
                all_prompt_templates, query_chunk
            )
            ...
            for _, sample_response in enumerate(chunk_results):
                if not sample_response:  # if sample prediction is an empty list :(
                    ...
                    all_gens_text = [
                        ...
                        for x in template_response[0]["choices"]
                    ]  # fuarr this is some high level programming
    ...
    async def _evaluate_candidate_points(
        ...
        if self.recalibrator is not None:
            recalibrated_res = self.recalibrator(
                y_mean, y_std, 0.68
            )  # 0.68 coverage for 1 std
