...
from llambo.generative_sm.generative_sm_utils import gen_prompt_tempates
...
class LLM_GEN_SM:
    def __init__(
        ...
        if rate_limiter is None:
            self.rate_limiter = RateLimiter(
                max_tokens=240000, time_frame=60, max_requests=2900
            )
        else:
            ...
            for retry in range(MAX_RETRIES):
                try:
                    ...
                    self.rate_limiter.add_request(
                        request_text=prompt, current_time=start_time
                    )
    ...
    def process_response(self, all_raw_response):
        ...
        for raw_response in all_raw_response:
            ...
            pred_index = min(
                (tokens.index(val) for val in ["0", "1"] if val in tokens), default=None
            )
