...
def bo_loop(
    ...
    for idx in range(n_repetitions):
        ...
        if bo_type in [
            ...
            BO_object = obtain_bo_object(
                bo_type, fun_to_evaluate, config_space, order_list, idx
            )
...
def run_all_models(
    ...
    for dict_template in all_dict_templates:
        ...
        list_config = template_obj.read_all_config(
            path_spec_configs, num_configs=n_repetitions
        )
...
# Function to run the Bayesian optimization process for models evaluated with Bayesmark
def run_tabular(
    list_model, list_data, all_dict_templates, n_repetitions, n_runs, n_init
):
